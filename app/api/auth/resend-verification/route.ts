...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📧 Reenviando correo de verificación personalizado a:', email)

    // Generar token de verificación usando Supabase Admin
    const supabase = await supabaseAdmin()
    
    // Generar nuevo link de confirmación
    ...
    if (authError) {
      console.error('❌ Error generando link de reenvío:', authError)
      return NextResponse.json(
    ...
    if (!result.success) {
      console.error('❌ Error reenviando correo de verificación:', result.error)
      return NextResponse.json(
        { error: result.error || 'Error al reenviar correo de verificación', details: result.data },
        { status: 500 }
      )
    }

    console.log('✅ Correo de verificación personalizado reenviado exitosamente:', result.data)
    return NextResponse.json({ 
      success: true, 
      id: result.data?.data?.[0]?.message_id || result.data?.message_id,
      message: 'Correo de verificación personalizado reenviado exitosamente'
    })

  } catch (error: any) {
    console.error('❌ Error en API de reenvío de verificación:', error)
