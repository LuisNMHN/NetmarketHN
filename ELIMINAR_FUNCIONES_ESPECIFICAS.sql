-- =========================================================
-- NMHN - ELIMINACIÓN ESPECÍFICA DE FUNCIONES
-- =========================================================
-- Script para identificar y eliminar cada versión específica de create_purchase_request
-- =========================================================

-- 1. Identificar TODAS las versiones existentes con sus firmas exactas
SELECT 
    routine_name,
    routine_type,
    data_type,
...
-- 3. ELIMINAR cada versión específica identificada
-- Versión 1: Función básica
...
-- Versión 5: Con diferentes tipos de parámetros
DROP FUNCTION IF EXISTS public.create_purchase_request(
    TEXT, DECIMAL, TEXT, TEXT, DECIMAL, TIMESTAMP WITH TIME ZONE,
...
-- 5. Verificar que se eliminaron todas las versiones
SELECT 
    routine_name,
    routine_type,
    data_type,
...
-- 6. Si aún existen funciones, usar eliminación por OID
DO $$
DECLARE
    func_oid OID;
BEGIN
    -- Eliminar todas las funciones create_purchase_request por OID
    FOR func_oid IN 
        SELECT p.oid 
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        ...
        AND n.nspname = 'public'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_oid;
    END LOOP;
END $$;
...
-- 10. Verificar que solo existe una versión
SELECT 
    routine_name,
    routine_type,
    data_type,
...
-- 11. Probar la función de generación de códigos
...
-- =========================================================
-- RESUMEN DE ELIMINACIÓN ESPECÍFICA
-- =========================================================
/*
ELIMINACIÓN ESPECÍFICA COMPLETADA:

1. ✅ Identificadas todas las versiones existentes
2. ✅ Eliminadas por firma específica
3. ✅ Eliminación por OID como respaldo
4. ✅ Recreadas desde cero

MÉTODOS DE ELIMINACIÓN:
- DROP FUNCTION con firma específica
- DROP FUNCTION por OID
