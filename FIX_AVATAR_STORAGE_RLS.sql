-- Script para corregir el error RLS al cargar avatares
...
-- 2. Verificar las políticas RLS actuales para el bucket kyc
...
DROP POLICY IF EXISTS "Enable insert for authenticated users on kyc bucket" ON storage.objects;
DROP POLICY IF EXISTS "Enable read access for authenticated users on kyc bucket" ON storage.objects;
DROP POLICY IF EXISTS "Enable update for authenticated users on kyc bucket" ON storage.objects;
DROP POLICY IF EXISTS "Enable delete for authenticated users on kyc bucket" ON storage.objects;

-- 4. Crear políticas RLS más permisivas para el bucket 'kyc'
-- Estas políticas permiten a usuarios autenticados subir archivos sin restricciones de carpeta
...
-- 7. Si RLS no está habilitado, habilitarlo
ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;

-- 8. Verificación final - mostrar todas las políticas de storage
SELECT 
  policyname,
  cmd,
  permissive,
