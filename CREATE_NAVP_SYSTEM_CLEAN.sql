-- Sistema NAVP (Network for Automated Value Payments) - VERSIÓN LIMPIA
-- OID/STC + OCR simple para validación de pagos por link/QR
-- Esta versión maneja políticas existentes
...
CREATE TABLE IF NOT EXISTS navp_payments (
    ...
    -- Referencias a transacciones (opcional)
    external_transaction_id TEXT -- ID de transacción externa
);
...
DROP POLICY IF EXISTS "Users can view own OIDs" ON navp_oids;
DROP POLICY IF EXISTS "Users can create own OIDs" ON navp_oids;
DROP POLICY IF EXISTS "Users can update own OIDs" ON navp_oids;

DROP POLICY IF EXISTS "Users can view own payments" ON navp_payments;
DROP POLICY IF EXISTS "Users can create payments" ON navp_payments;
DROP POLICY IF EXISTS "Users can update own payments" ON navp_payments;

DROP POLICY IF EXISTS "Users can view OCR scans" ON navp_ocr_scans;
DROP POLICY IF EXISTS "Users can create OCR scans" ON navp_ocr_scans;

DROP POLICY IF EXISTS "Users can view validations" ON navp_validations;

DROP POLICY IF EXISTS "Users can view processing logs" ON navp_processing_logs;
...
-- Función para completar pago (versión simplificada)
...
BEGIN
    ...
    -- Actualizar información del pagador y completar pago
    UPDATE navp_payments 
    SET 
