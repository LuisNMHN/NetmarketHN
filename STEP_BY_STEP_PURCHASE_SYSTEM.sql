-- =========================================================
-- SCRIPT PASO A PASO PARA CREAR EL SISTEMA DE COMPLETAR COMPRA
-- =========================================================
-- Este script ejecuta cada paso individualmente para evitar errores
-- =========================================================

-- =========================================================
-- PASO 1: CREAR TABLA PURCHASE_TRANSACTIONS
-- =========================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'purchase_transactions') THEN
        RAISE NOTICE 'Tabla purchase_transactions creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: No se pudo crear la tabla purchase_transactions';
    END IF;
END $$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transaction_steps') THEN
        RAISE NOTICE 'Tabla transaction_steps creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: No se pudo crear la tabla transaction_steps';
    END IF;
END $$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transaction_documents') THEN
        RAISE NOTICE 'Tabla transaction_documents creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: No se pudo crear la tabla transaction_documents';
    END IF;
END $$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transaction_notifications') THEN
        RAISE NOTICE 'Tabla transaction_notifications creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: No se pudo crear la tabla transaction_notifications';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transaction_disputes') THEN
        RAISE NOTICE 'Tabla transaction_disputes creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: No se pudo crear la tabla transaction_disputes';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Función validate_status_transition creada correctamente';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Función create_purchase_transaction creada correctamente';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Función update_transaction_status creada correctamente';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Función cleanup_expired_transactions creada correctamente';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Todos los triggers creados correctamente';
END $$;
...
-- Verificar que RLS se configuró
DO $$
BEGIN
...
DO $$
BEGIN
    RAISE NOTICE 'Sistema de completar compra creado exitosamente!';
END $$;

-- =========================================================
-- NOTAS IMPORTANTES:
-- =========================================================
-- 1. Este script ejecuta cada paso individualmente
-- 2. Incluye verificaciones después de cada paso
-- 3. Evita errores de dependencias
-- 4. Confirma que todo se creó correctamente
-- 5. La columna payment_deadline se crea en el PASO 1
-- =========================================================
