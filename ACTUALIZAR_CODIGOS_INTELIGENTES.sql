-- =========================================================
-- NMHN - ACTUALIZAR CÓDIGOS ÚNICOS INTELIGENTES
-- =========================================================
-- Script para actualizar la función de generación de códigos únicos
...
-- 3. Crear función mejorada que no reutiliza códigos eliminados
...
BEGIN
    ...
    unique_code := 'NMHN-' || today_date || '-' || LPAD(next_number::TEXT, 6, '0');
    
    -- Verificar que el código no existe (doble verificación)
    WHILE EXISTS (SELECT 1 FROM purchase_requests pr WHERE pr.unique_code = generate_unique_code_safe.unique_code) 
    AND attempt_count < max_attempts LOOP
    ...
    IF attempt_count >= max_attempts THEN
        unique_code := 'NMHN-' || today_date || '-' || LPAD(EXTRACT(EPOCH FROM NOW())::INTEGER % 1000000, 6, '0');
    END IF;
...
1. ✅ Formato Actualizado:
   - Antes: NMHN-YYYYMMDD-XXXXXX
   - Ahora: NMHN-YYMMDD-000000

2. ✅ Generación Inteligente:
   - No reutiliza códigos eliminados
   - Solo cuenta códigos existentes actualmente
   - Incremento secuencial correcto

3. ✅ Validación Robusta:
   - Verifica formato con regex
   - Doble verificación de unicidad
